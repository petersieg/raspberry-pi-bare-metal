...
void game(SDL_Window *screen,uint *state,uint *grapset,uint *fullscreen) {
    ...
    SDL_Surface * rendererb = SDL_GetWindowSurface(screen);
    ...
    /* Loading PNG */
    SDL_Texture *tiles = SDL_CreateTexture(renderer,SDL_PIXELFORMAT_ABGR8888,SDL_TEXTUREACCESS_STATIC,1000,240);
    ...
    SDL_Surface *tilesb = IMG_Load_RW(rw,1);
    SDL_SetColorKey(tilesb, SDL_TRUE, SDL_MapRGB(tilesb->format, 0, 0, 0) );
    tiles = SDL_CreateTextureFromSurface(renderer, tilesb);

    rw = SDL_RWFromMem(&_binary_graphics_fonts_png_start,
        (unsigned int)&_binary_graphics_fonts_png_end - (unsigned int)&_binary_graphics_fonts_png_start);
    SDL_Surface *fonts = IMG_Load_RW(rw,1);
    ...
    while (exit != 1) {
        ...
        /* Draw screen */
        drawscreen(rendererb,stagedata,tilesb,room,counter,changeflag,fx,changetiles);

        /* Draw statusbar */
        if (room[0] != 4)
            statusbar(rendererb,tilesb,room,jean.state[0],jean.state[1],fonts,changetiles);

        /* Draw Jean */
        if (jean.flags[6] < 8)
            drawjean (rendererb,tilesb,&jean,counter,fx,changetiles);
        ...
        if (enemies.type[0] > 0) {
            ...
            if ((room[0] == 5) || (room[0] == 6))
                crusaders (&enemies,rendererb,tilesb,counter,room,changetiles);
            if (room[0] == 10)
                dragon (&enemies,rendererb,tilesb,counter,proyec,fx,changetiles);
            if (room[0] == 11)
                fireball (&enemies,rendererb,tilesb,counter,jean,stagedata,changetiles);
            if (room[0] == 14)
                plants (&enemies,rendererb,tilesb,counter,proyec,fx,changetiles);
            if (room[0] == 9)
                drawrope (enemies,rendererb,tilesb,changetiles);
            if (room[0] == 18)
                death (&enemies,rendererb,tilesb,counter,proyec,stagedata,fx,changetiles);
            if ((room[0] == 24) && (enemies.type[0] == 18))
                satan (&enemies,rendererb,tilesb,counter,proyec,fx,changetiles);
            if ((room[0] == 24) && (jean.flags[6] == 5))
                crusaders (&enemies,rendererb,tilesb,counter,room,changetiles);
            drawenemies (&enemies,rendererb,tilesb,fx,changetiles);
        }

        /* Shoots */
        if ((proyec[0] > 0) && ((room[0] == 17) || (room[0] == 20) || (room[0] == 21) || (room[0] == 22)))
          drawshoots (proyec,tilesb,rendererb,&enemies,changetiles);
